...
class Helper {
	 ...
	 */
	public static function validateCoupon( $coupon, $spend, $emails = array() ) {
		...
		if ( count( $emails ) ) {
			...
			if ( count( $array ) ) {
				...
				if ( ! $allowed ) {
					return 'Sorry it seems the coupon "' . $coupon->code . '" is not allowed';
				}
			}
		}
	...
	public static function getAvailablePaymentMethods() {
		$pay_tms     = PaymentMethod::select( array( 'id', 'name', 'slug' ) )->where( 'enabled', true )->get();
		$pay_methods = collect( array() );
		$pay_tms->each(
			function ( $item, $key ) use ( $pay_methods ) {
				$payment_method_details = collect( array() );
