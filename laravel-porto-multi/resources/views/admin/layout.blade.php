...
<body>
    ...
    <script>
        var portoStorage = JSON.parse(window.localStorage.getItem('porto-multi'));
        if (portoStorage) {
            portoStorage.user = {
            ...
            window.localStorage.setItem('porto-multi', JSON.stringify(portoStorage));
        } else {
            window.localStorage.setItem('porto-multi', JSON.stringify({
                user: {
                    data: JSON.parse(`{!! json_encode(Auth::user()) !!}`),
                    token: '{{ csrf_token() }}'
                }
